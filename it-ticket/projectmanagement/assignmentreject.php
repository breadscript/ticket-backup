...
<div class="table-responsive">
	<table class="table table-striped table-bordered table-hover" id="dataTables-taskpmreject">
		<thead>
			<tr>
				<th>Task ID</th>
				<th>Created By</th>
		...
		if($myusergroupid == 3 || $myusergroupid == 2 || $myusergroupid == 1){
			...
			if(!empty($taskIds)) {
				...
				$myquery = "SELECT pm_projecttasktb.id,pm_projecttasktb.subject,pm_projecttasktb.description,pm_projecttasktb.deadline,
						...
						WHERE pm_projecttasktb.istask = '1' AND pm_projecttasktb.id IN ($taskIdList) AND pm_projecttasktb.statusid = '2'
			...
			$myquery = "SELECT pm_projecttasktb.id,pm_projecttasktb.srn_id,pm_projecttasktb.subject,pm_projecttasktb.description,pm_projecttasktb.deadline,
						...
						LEFT JOIN sys_priorityleveltb ON sys_priorityleveltb.id=pm_projecttasktb.priorityid
						LEFT JOIN sys_projecttb ON sys_projecttb.id=pm_projecttasktb.projectid
						WHERE pm_projecttasktb.istask = '1' AND pm_projecttasktb.statusid = '2'
						ORDER BY pm_projecttasktb.datetimecreated ASC";
			$myresult = mysqli_query($conn, $myquery);
		}
		...
		if($myresult && mysqli_num_rows($myresult) > 0) {
			...
			echo '<tr><td colspan="12" class="text-center">No rejected tasks found</td></tr>';
		}
		?>
		</tbody>
	</table>
...
<script type="text/javascript">
 $(document).ready(function() {
       $('#dataTables-taskpmreject').DataTable({
            "aaSorting": [],
			"scrollCollapse": true,	
			"autoWidth": false,
			"responsive": true,
