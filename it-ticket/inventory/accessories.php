...
?>
<style>
/* Compact table styling for Accessories */
#dataTables-accessories {
    font-size: 12px;
}

#dataTables-accessories td {
    padding: 6px 4px;
...
<table class="table table-striped table-bordered table-hover" id="dataTables-accessories">
    <thead>
        <tr>
    ...
    // Query to fetch Accessories inventory items
    $query = "SELECT *
    FROM inv_inventorylisttb
    WHERE statusid = 0 
    AND asset_type != 'PC DESKTOP' AND asset_type != 'Smartphone' AND asset_type != 'Laptop' AND asset_type != 'Tablet' AND asset_type != 'Printers'
    ...
    if($result && mysqli_num_rows($result) > 0) {
        ...
        // No data found
        echo '<tr><td colspan="19" class="text-center"><span style="color: red; font-weight: bold;">No Accessories inventory items found</span></td></tr>';
    }
    ?>
    </tbody>
</table>
...
<script type="text/javascript">
 $(document).ready(function() {
       $('#dataTables-accessories').DataTable({
            "aaSorting": [],
            "scrollCollapse": true, 
            "autoWidth": false,
            "responsive": true,
