...
$ticketSubjectUp=urldecode($_GET['ticketSubjectUp2']);

// Debug logging
error_log("Starting ticket update process");
...
$auditRemarks3 = "ATTEMPT TO UPDATE TICKET, DUPLICATE"." | ".$ticketSubjectUp;
    ...
    if($ticketStatus != 6 && $ticketStatus != 2 && $ticketStatus != 3 && $ticketStatus != 4){
        ...
        if($myresultxx){
            ...
            try {
                $emailSent = sendTicketUpdateNotification($conn, $ticketIdUp, $ticketSubjectUp, $ticketStatus, $ticketUseridUp);
                if (!$emailSent) {
                    error_log("Failed to send ticket update notification email");
                }
            } catch (Exception $e) {
                error_log("Error sending ticket update notification: " . $e->getMessage());
            }
