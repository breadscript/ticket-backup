...
<html lang="en">
                        ...
                        <div class="form-group" style="max-width: 200px;">
															<label class="block clearfix">
																<span class="block input-icon input-icon-left">
																	<select class="form-control" id="taskAssignee" name="taskAssignee" style="width: 100%;" onchange="updateDashboardCounts(this.value)">
																	<option value="all">All Assignees</option>
																		<?php 
																			$query_assignee = "SELECT id,user_firstname,user_lastname FROM sys_usertb WHERE user_statusid = '1' AND user_groupid IN (2, 3)";
																			$result_assignee = mysqli_query($conn, $query_assignee);
																			while($row_assignee = mysqli_fetch_assoc($result_assignee)){
																				$assigneeFname=mb_convert_case($row_assignee['user_firstname'], MB_CASE_TITLE, "UTF-8");
																				$assigneeLname=mb_convert_case($row_assignee['user_lastname'], MB_CASE_TITLE, "UTF-8");
																				$assigneeCompleteName = $assigneeFname." ".$assigneeLname;
																				$assigneeId=$row_assignee['id'];
																		?>
																		<option value="<?php echo $assigneeId;?>"><?php echo $assigneeCompleteName;?></option>
																		<?php } ?>
																	</select>
																</span>
															</label>
	...
	<script>
	function updateDashboardCounts(assigneeId) {
		// Show loading indicator if needed
		console.log('Updating for assignee:', assigneeId);
		
		// Make AJAX call to get updated counts
		fetch('get_dashboard_counts.php?assignee=' + assigneeId)
			.then(response => {
				if (!response.ok) {
			...
			.then(data => {
				...
				try {
					// Update ticket counts - first infobox in first container
					const ticketContainer = document.querySelectorAll('.infobox-container')[0];
					const assignmentContainer = document.querySelectorAll('.infobox-container')[1];

					// Update total tickets
					ticketContainer.querySelector('.infobox-green .infobox-data-number').textContent = data.tickets.total;

					// Get all ticket status boxes
					const ticketBoxes = ticketContainer.querySelectorAll('.infobox');
					
					// Update ticket counts (starting from index 1 as index 0 is the total count box)
					if (ticketBoxes.length >= 7) {
						ticketBoxes[1].querySelector('.percent').textContent = data.tickets.new;
						ticketBoxes[2].querySelector('.percent').textContent = data.tickets.in_progress;
						ticketBoxes[3].querySelector('.percent').textContent = data.tickets.pending;
						ticketBoxes[4].querySelector('.percent').textContent = data.tickets.done;
						ticketBoxes[5].querySelector('.percent').textContent = data.tickets.rejected;
						ticketBoxes[6].querySelector('.percent').textContent = data.tickets.cancelled;
					}

					// Update total assignments
					assignmentContainer.querySelector('.infobox-blue .infobox-data-number').textContent = data.assignments.total;

					// Get all assignment status boxes
					const assignmentBoxes = assignmentContainer.querySelectorAll('.infobox');
					
					// Update assignment counts (starting from index 1 as index 0 is the total count box)
					if (assignmentBoxes.length >= 7) {
						assignmentBoxes[1].querySelector('.percent').textContent = data.assignments.new;
						assignmentBoxes[2].querySelector('.percent').textContent = data.assignments.in_progress;
						assignmentBoxes[3].querySelector('.percent').textContent = data.assignments.pending;
						assignmentBoxes[4].querySelector('.percent').textContent = data.assignments.done;
						assignmentBoxes[5].querySelector('.percent').textContent = data.assignments.rejected;
						assignmentBoxes[6].querySelector('.percent').textContent = data.assignments.cancelled;
					}

					// Update classification tables
					updateClassificationTables(data.classifications);
				} catch (error) {
	...
	function updateClassificationTables(classifications) {
		try {
			// Get the classification table bodies - using more specific selectors
			const ticketTable = document.querySelector('.col-sm-12.col-md-6 .widget-box.transparent:first-child table tbody');
			const assignmentTable = document.querySelector('.col-sm-12.col-md-6:last-child .widget-box.transparent table tbody');

			if (!ticketTable || !assignmentTable) {
				console.error('Classification tables not found');
				return;
			}

			// Update ticket classifications
			if (classifications.tickets && classifications.tickets.length > 0) {
				const ticketRows = ticketTable.getElementsByTagName('tr');
				classifications.tickets.forEach((classification, index) => {
					if (ticketRows[index]) {
						const cells = ticketRows[index].getElementsByTagName('td');
						if (cells.length >= 4) {
							// Update total count
							const totalCell = cells[1].querySelector('b.green');
							if (totalCell) totalCell.textContent = classification.total;

							// Update ongoing count
							const ongoingCell = cells[2].querySelector('b.red');
							if (ongoingCell) ongoingCell.textContent = classification.ongoing;

							// Update closed count
							const closedCell = cells[3].querySelector('span.label');
							if (closedCell) closedCell.textContent = classification.closed;
						}
					}
				});
			}

			// Update assignment classifications
			if (classifications.assignments && classifications.assignments.length > 0) {
				const assignmentRows = assignmentTable.getElementsByTagName('tr');
				classifications.assignments.forEach((classification, index) => {
					if (assignmentRows[index]) {
						const cells = assignmentRows[index].getElementsByTagName('td');
						if (cells.length >= 4) {
							// Update total count
							const totalCell = cells[1].querySelector('b.green');
							if (totalCell) totalCell.textContent = classification.total;
			...
			console.error('Error updating classification tables:', error);
		}
	}
	</script>
</body>
