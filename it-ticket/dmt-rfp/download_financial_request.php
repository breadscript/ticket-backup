<?php
// Download financial request as HTML file
// This file handles the download functionality by generating the HTML content and forcing the browser to download it
...
$docTypeParam = isset($_POST['doc_type']) ? (string)$_POST['doc_type'] : '';
$docNumberParam = isset($_POST['doc_number']) ? (string)$_POST['doc_number'] : '';
...
// admin
if (!$canDownload && strtolower((string)$__SESSION_USER['role']) === 'admin') {
  $canDownload = true;
}
// workflow actor (process table)
if (!$canDownload && isset($mysqlconn) && $mysqlconn) {
  ...
  if ($chStmt) {
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
      if (__matches_actor_p($row['actor_id'] ?? '', $__SESSION_USER)) { $canDownload = true; break; }
    }
...
if (!$canDownload && isset($mysqlconn) && $mysqlconn) {
  ...
  if ($checkStmt) {
    ...
    if (!$hasWorkflowRecords) { $canDownload = true; }
  }
}
...
// Generate filename
$filename = 'financial_request_' . $requestType . '_' . $fr['doc_number'] . '_' . date('Y-m-d_H-i-s') . '.pdf';

// Set headers for download (will be overridden by mPDF for PDF generation)
...
// Generate the appropriate HTML based on request type
if ($requestType === 'ERL') {
  generateERL_HTML($fr, $items, $breakdowns, $mysqlconn);
} elseif ($requestType === 'ERGR') {
  generateERGR_HTML($fr, $items, $breakdowns, $mysqlconn);
} elseif ($requestType === 'RFP') {
  generateRFP_HTML($fr, $items, $breakdowns, $mysqlconn);
} else {
  generateGeneric_HTML($fr, $items, $breakdowns, $mysqlconn);
}
...
try {
  ...
  // Check if mPDF class exists (using newer version 8.x class name)
  if (!class_exists('\Mpdf\Mpdf')) {
    throw new Exception('mPDF class not found');
  }
  
  // Create mPDF instance with proper configuration (newer version 8.x)
  $mpdf = new \Mpdf\Mpdf([
    'mode' => 'utf-8',
    'format' => 'A4',
    'orientation' => $requestType === 'ERL' || $requestType === 'ERGR' ? 'L' : 'P',
    'margin_left' => 15,
    'margin_right' => 15,
    'margin_top' => 16,
    'margin_bottom' => 16,
  ...
  // Generate filename
  $filename = $requestType . '_' . $docNumberParam . '_' . date('Y-m-d_H-i-s') . '.pdf';
  ...
  header('Content-Disposition: attachment; filename="' . $requestType . '_' . $docNumberParam . '_' . date('Y-m-d_H-i-s') . '.html"');
  echo $html;
  exit;
}
...
// Include the same generation functions as print_financial_request.php
function generateERL_HTML($fr, $items, $breakdowns, $mysqlconn = null) {
  // Same content as generateERL_PDF but without the print buttons
  ...
  <head>
    ...
    <style>
      ...
      .date { text-align: right; margin-bottom: 8px; font-size: 9px; }
      .form-section { margin-bottom: 8px; }
      .form-row { margin-bottom: 4px; }
      .form-label { display: inline-block; width: 150px; font-weight: bold; }
      .form-value { display: inline-block; width: 300px; border-bottom: 1px solid #000; min-height: 14px; }
      table { width: 100%; border-collapse: collapse; margin: 8px 0; }
      th, td { border: 1px solid #000; padding: 4px; text-align: left; font-size: 8px; }
      th { background-color: #f0f0f0; font-weight: bold; }
      .total-row { font-weight: bold; background-color: #f0f0f0; }
      .notes { margin-top: 8px; font-size: 7px; line-height: 1.3; }
      .signatures { margin-top: 12px; }
      .signature-row { display: flex; justify-content: space-between; margin-bottom: 8px; }
      .signature-box { text-align: center; width: 180px; font-size: 8px; }
      .signature-line { border-bottom: 1px solid #000; margin: 15px 0 5px 0; }
    </style>
  </head>
  <body>
    <div class="header">
    ...
    <div class="signatures">
      ...
      <div class="signature-box">
        ...
        <?php 
        // Display first 2 approvers (to complete first row of 3)
        $sequenceTitles = [
          2 => 'Cost Center Head',
          3 => 'Accounting',
          4 => 'Accounting', 
        ...
        // Fill remaining slots in first row to reach 3 columns
        for ($i = 2; $i <= 6 && $count < 2; $i++) {
          if (!in_array($i, $displayedSequences) && isset($sequenceTitles[$i])) {
            $displayedSequences[] = $i;
            $count++;
            echo '<div class="signature-box">';
            echo '<div class="signature-line"></div>';
            echo '<div>' . htmlspecialchars($sequenceTitles[$i]) . ':</div>';
            echo '</div>';
          }
        }
        ?>
      ...
      <div class="signature-row">
        <?php 
        // Display remaining approvers (3 columns for second row)
        $count = 0;
        foreach ($approvers as $approver) {
          $seq = $approver['sequence'];
        ...
        // Fill remaining slots in second row to reach 3 columns
...
function generateERGR_HTML($fr, $items, $breakdowns, $mysqlconn = null) {
  // Same content as generateERGR_PDF but without the print buttons
  $company = (string)($fr['company_name'] ?? $fr['company'] ?? '');
  $companyCode = (string)($fr['company'] ?? '');
        ...
        <div class="header-left">
          <div class="logo">
            <?php if ($logoExists) { ?>
              <img src="<?php echo htmlspecialchars($logoPath); ?>" alt="<?php echo htmlspecialchars($company); ?>" style="max-width: 80px; max-height: 80px; border-radius: 50%;">
            <?php } else { ?>
              <div style="font-size: 10px; text-align: center; font-weight: bold;">LOGO</div>
            <?php } ?>
          </div>
          <div class="company-name"><?php echo htmlspecialchars($company); ?></div>
        </div>
      <!-- Revolving Fund panel intentionally not shown for ERGR -->
    </div>
...
function generateRFP_HTML($fr, $items, $breakdowns, $mysqlconn = null) {
  // Same content as generateRFP_PDF but without the print buttons
  $company = (string)($fr['company_name'] ?? $fr['company'] ?? '');
  $companyCode = (string)($fr['company'] ?? '');
  $payee = (string)($fr['payee'] ?? ''); // Use payee instead of user name
  $department = (string)($fr['department_name'] ?? '');
  ...
  <head>
    ...
    <style>
      body { font-family: Arial, sans-serif; font-size: 11px; margin: 30px; line-height: 1.2; }
      .document-container { border: 1px solid #000; padding: 10px; max-width: 700px; margin: 0 auto; }
      .header { text-align: center; margin-bottom: 25px; }
      .logo { width: 70px; height: auto; margin: 0 auto 5px; border: none; }
      .logo img { max-width: 58px; max-height: 58px; border-radius: 50%; }
      .company-name { font-size: 14px; font-weight: bold; margin-bottom: 15px; letter-spacing: 1px; }
      .document-title { font-size: 13px; font-weight: bold; margin-bottom: 20px; }
      .form-section { margin-bottom: 20px; }
      .form-row { display: flex; justify-content: space-between; margin-bottom: 8px; }
      .form-left { flex: 1; display: flex; align-items: center; }
      .form-right { flex: 1; display: flex; align-items: center; justify-content: flex-end; }
      .form-label { width: 100px; font-weight: normal; }
      .form-underline { border-bottom: 1px solid #000; flex: 1; min-width: 120px; display: inline-block; }
      .form-underline-right { border-bottom: 1px solid #000; min-height: 18px; min-width: 120px; padding: 2px 5px; }
      .main-table { width: 100%; border: 1px solid #000; border-collapse: collapse; }
      .main-table th, .main-table td { border: 1px solid #000; padding: 6px; font-size: 11px; }
      .particulars-col { width: 70%; }
      .amount-col { text-align: right; }
      .particulars-content { min-height: 100px; padding: 5px; }
      .amount-content { min-height: 100px; padding: 5px; text-align: right; }
      .signature-row { width: 100%; border-collapse: collapse; }
      .signature-row td { border: 1px solid #000; padding: 8px; vertical-align: bottom; text-align: center; height: 60px; }
      .total-cell { width: 25%; text-align: left; vertical-align: bottom; }
      .signature-cell { width: 25%; }
  ...
  <body>
    <div class="document-container">
      <div class="header">
        <div class="logo">
          <?php if ($logoExists) { ?>
            <img src="<?php echo htmlspecialchars($logoPath); ?>" alt="<?php echo htmlspecialchars($company); ?>" style="max-width: 58px; max-height: 58px; border-radius: 50%;">
          <?php } else { ?>
...
function generateGeneric_HTML($fr, $items, $breakdowns, $mysqlconn = null) {
  $requestType = (string)($fr['request_type'] ?? '');
  $company = (string)($fr['company_name'] ?? $fr['company'] ?? '');
  $companyCode = (string)($fr['company'] ?? '');
