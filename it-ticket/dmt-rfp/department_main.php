...
if (isset($mysqlconn) && $mysqlconn && $search !== '') {
  ...
  $like = "'%" . $esc . "%'";
  $whereClause = "WHERE department_name LIKE $like OR department_code LIKE $like OR department_id LIKE $like OR company_code LIKE $like";
}
...
if (isset($mysqlconn) && $mysqlconn) {
  // Total count
  if ($cntRes = mysqli_query($mysqlconn, "SELECT COUNT(*) AS cnt FROM department $whereClause")) {
  ...
  $limit = (int)$perPage;
  $off = (int)$offset;
  $listSql = "SELECT id, company_code, department_code, department_id, department_name, created_at FROM department $whereClause ORDER BY created_at DESC LIMIT $limit OFFSET $off";
  if ($listResult = mysqli_query($mysqlconn, $listSql)) {
    while ($row = mysqli_fetch_assoc($listResult)) { $departments[] = $row; }
  }
}
?>
          ...
          <div class="">
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Departments <small>view and manage</small></h2>
                    <ul class="nav navbar-right panel_toolbox">
                      <li>
                        <button type="button" class="btn btn-success btn-sm" id="createBtn" name="create" onclick="window.location.href='department_form.php'">
                        ...
                        <div class="col-sm-6">
                          <form method="get" class="form-inline" autocomplete="off" style="margin-bottom:8px;">
                            <div class="form-group">
                              <input type="text" name="q" class="form-control input-sm" placeholder="Search departments..." value="<?php echo htmlspecialchars($search, ENT_QUOTES, 'UTF-8'); ?>">
                            </div>
                            <button type="submit" class="btn btn-primary btn-sm"><i class="fa fa-search"></i> Search</button>
                            <?php if ($search !== '') { ?>
                              <a class="btn btn-default btn-sm" href="department_main.php"><i class="fa fa-undo"></i> Reset</a>
                            <?php } ?>
                          </form>
                        </div>
                        ...
                        <tbody>
                          ...
                          <?php } else { ?>
                            <?php foreach ($departments as $d) { 
                              $id = (int)($d['id'] ?? 0);
                              $deptName = htmlspecialchars($d['department_name'] ?? '', ENT_QUOTES, 'UTF-8');
                              $deptCode = htmlspecialchars($d['department_code'] ?? '', ENT_QUOTES, 'UTF-8');
                              $deptId  = htmlspecialchars($d['department_id'] ?? '', ENT_QUOTES, 'UTF-8');
                              $companyCode = htmlspecialchars($d['company_code'] ?? '', ENT_QUOTES, 'UTF-8');
                              $created = htmlspecialchars($d['created_at'] ?? '', ENT_QUOTES, 'UTF-8');
                            ...
                            ?>
                              <tr>
                                ...
                                <td>
                                  <button type="button" class="btn btn-xs btn-info" onclick="window.location.href='department_view.php?id=<?php echo $id; ?>'">
                                    <i class="fa fa-eye"></i> View
                                  </button>
                                  <button type="button" class="btn btn-xs btn-warning" onclick="window.location.href='department_form.php?id=<?php echo $id; ?>'">
