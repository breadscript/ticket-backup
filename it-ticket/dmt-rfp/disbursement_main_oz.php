...
//     }
// }
// Check if current user is the special admin user who can see all requests
$currentUserId = isset($_SESSION['userid']) ? (int)$_SESSION['userid'] : 0;
$isAdminUser = false;
if ($currentUserId > 0 && isset($mysqlconn) && $mysqlconn) {
  $adminCheckSql = "SELECT emailadd FROM sys_usertb WHERE id = ? AND emailadd = 'MMREDONDO@CARMENSBEST.COM.PH' LIMIT 1";
  if ($stmtAdmin = mysqli_prepare($mysqlconn, $adminCheckSql)) {
    mysqli_stmt_bind_param($stmtAdmin, 'i', $currentUserId);
    mysqli_stmt_execute($stmtAdmin);
    $adminResult = mysqli_stmt_get_result($stmtAdmin);
    $isAdminUser = (mysqli_fetch_assoc($adminResult) !== null);
    mysqli_stmt_close($stmtAdmin);
  }
}

// Fetch counts per request type (RFP, ERL, ERGR) - all users for admin, current user only for others
$requestTypeToCount = ['RFP' => 0, 'ERL' => 0, 'ERGR' => 0];
if (isset($mysqlconn) && $mysqlconn) {
  if ($isAdminUser) {
    $countSql = "SELECT request_type, COUNT(*) AS total FROM financial_requests GROUP BY request_type";
    ...
    $countSql = "SELECT request_type, COUNT(*) AS total FROM financial_requests WHERE created_by_user_id = ? GROUP BY request_type";
    if ($stmtCnt = mysqli_prepare($mysqlconn, $countSql)) {
...
// Fetch available requests to display in the table - all users for admin, current user only for others
$requests = [];
if (isset($mysqlconn) && $mysqlconn) {
  ...
  $listSql = "
    SELECT fr.id,
           ...
           (
             SELECT CASE 
               WHEN EXISTS (
                 SELECT 1 FROM work_flow_process wcc
                 WHERE wcc.doc_type = fr.request_type
                   AND wcc.doc_number = fr.doc_number
                   AND wcc.sequence = 2
                   AND (
                     UPPER(wcc.status) = 'APPROVED'
                     OR UPPER(wcc.status) = 'DONE'
                     OR wcc.status LIKE 'Approved%'
                   )
               ) THEN 1
               ELSE 0
             END
           ) AS cost_center_approved,
  ...
  // Add WHERE clause conditionally based on admin status
  if (!$isAdminUser) {
    $listSql .= " WHERE fr.created_by_user_id = ?";
  }
  
  $listSql .= " ORDER BY fr.created_at DESC";
...
foreach ($requests as $req) {
  ...
  $costCenterApproved = isset($req['cost_center_approved']) ? (int)$req['cost_center_approved'] : 0;
  ...
  } elseif ($statusRaw === 'declined' || $statusRaw === 'rejected' || strpos($statusRaw, 'return') !== false) {
    // Show declined/rejected requests and return to requestor requests in pending (can be edited/resubmitted)
    $pendingRequests[] = $req;
  } elseif ($costCenterApproved === 1) {
    // Only move to ongoing if Cost Center Head (sequence 2) has approved
    $ongoingRequests[] = $req;
  } else {
    // Newly created requests and those waiting for Cost Center Head approval stay in pending
    $pendingRequests[] = $req;
  }
}
?>
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">
                    <div style="margin-bottom:8px;">
                      ...
                      <?php if (isset($_GET['debug']) && $_GET['debug'] === '1') { ?>
                        <div style="background: #f0f0f0; padding: 10px; margin: 10px 0; border: 1px solid #ccc; font-size: 12px;">
                          ...
                          Pending: <?php echo count($pendingRequests); ?> (Newly created + waiting for Cost Center approval)<br>
                          On-Going: <?php echo count($ongoingRequests); ?> (Cost Center approved, in further approval process)<br>
                          ...
                          <?php if (!empty($requests)) { ?>
                            ...
                            <?php 
                              ...
                              foreach ($requests as $req) {
                                ...
                                $costCenterApproved = isset($req['cost_center_approved']) ? (int)$req['cost_center_approved'] : 0;
                                echo "ID: " . $req['id'] . " - Status: '" . ($req['status_to_show'] ?? 'NULL') . "' (Original: '" . ($req['status'] ?? 'NULL') . "') - Cost Center Approved: " . $costCenterApproved . "<br>";
                                $sampleCount++;
                              }
                            ?>
                          <?php } ?>
                    ...
                    <ul class="nav nav-tabs" role="tablist">
                      <li role="presentation" class="active"><a href="#tab-pending" aria-controls="tab-pending" role="tab" data-toggle="tab" title="Newly created requests and those waiting for Cost Center Head approval">Pending <span class="badge"><?php echo count($pendingRequests); ?></span></a></li>
                      <li role="presentation"><a href="#tab-ongoing" aria-controls="tab-ongoing" role="tab" data-toggle="tab" title="Requests approved by Cost Center Head, now in further approval process">On-Going <span class="badge"><?php echo count($ongoingRequests); ?></span></a></li>
                      <li role="presentation"><a href="#tab-approved" aria-controls="tab-approved" role="tab" data-toggle="tab" title="Fully approved requests">Approved <span class="badge"><?php echo count($approvedRequests); ?></span></a></li>
                      <li role="presentation"><a href="#tab-cancelled" aria-controls="tab-cancelled" role="tab" data-toggle="tab" title="Cancelled requests">Cancelled <span class="badge"><?php echo count($cancelledRequests); ?></span></a></li>
                    </ul>
                    ...
                    <div class="tab-content" style="margin-top:10px;">
                      ...
                      <div role="tabpanel" class="tab-pane fade in active" id="tab-pending">
                        <div class="alert alert-info" style="margin-bottom: 15px;">
                          <i class="fa fa-info-circle"></i> <strong>Note:</strong> This tab shows newly created requests and those waiting for Cost Center Head approval. Requests will move to "On-Going" once the Cost Center Head has approved them.
                        </div>
                                    ...
                                    <td>
                                      <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu" style="min-width: 120px; max-width: 120px;">
                                          <li><a href="disbursement_view_oz.php?id=<?php echo $id; ?>"><i class="fa fa-eye"></i> View</a></li>
                                          <?php if ($canEdit) { ?>
                                          <li><a href="disbursement_edit_form.php?id=<?php echo $id; ?>"><i class="fa fa-pencil"></i> Edit</a></li>
                                          <?php } ?>
                      ...
                      <div role="tabpanel" class="tab-pane fade" id="tab-ongoing">
                        <div class="alert alert-success" style="margin-bottom: 15px;">
                          <i class="fa fa-check-circle"></i> <strong>Note:</strong> This tab shows requests that have been approved by the Cost Center Head and are now in the further approval process (Accounting, Controller, Cashier, etc.).
                        </div>
                        <div class="table-responsive" style="overflow-x: auto;">
                      ...
                      <div role="tabpanel" class="tab-pane fade" id="tab-cancelled">
                        <div class="alert alert-info" style="margin-bottom: 15px;">
                          <i class="fa fa-info-circle"></i> <strong>Note:</strong> This tab shows cancelled requests only. Declined requests appear in the Pending tab for further action.
                        </div>
                        <div class="table-responsive" style="overflow-x: auto;">
    ...
    <script>
      (function() {
        ...
        function showFilterNotification(filterType) {
          ...
          notification.innerHTML = '<strong>Filter Active:</strong> Showing only ' + filterType + ' requests. <strong>Note:</strong> Declined requests will appear in the "Cancelled" tab. <button type="button" class="btn btn-xs btn-default" onclick="hideFilterNotification()" style="float: right; margin-top: -5px;">×</button>';
        ...
        function applyFilter(filter) {
          ...
          // Ensure the cancelled tab is visible and shows the correct count
          // This helps when declined requests are filtered by type
          if (filter && filter !== '') {
            // When filtering, make sure all tabs are accessible
            var cancelledTab = document.querySelector('a[href="#tab-cancelled"]');
            if (cancelledTab) {
              var cancelledBadge = cancelledTab.querySelector('.badge');
              if (cancelledBadge) {
                cancelledBadge.textContent = tabCounts.cancelled;
              }
            }
          }
        }
            ...
            resetBtn.addEventListener('click', function() {
              ...
              setTimeout(function() {
                updateTabCounts({
                  ...
                  ongoing: document.querySelectorAll('#tableOngoing tbody tr:not(.js-placeholder)').length,
