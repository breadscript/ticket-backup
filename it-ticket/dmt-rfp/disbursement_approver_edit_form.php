...
$__CLEAN_EDIT_URL = 'disbursement_approver_edit_form.php';
...
if ($editId > 0 && isset($mysqlconn) && $mysqlconn) {
  ...
  $canEdit = false;
  if ($er && isset($mysqlconn) && $mysqlconn) {
    ...
    $creatorName = trim((($er['created_by_name'] ?? '')));
    if (strcasecmp($creatorName, $currentUserFullName) === 0 || 
        strcasecmp($creatorName, $currentUserUsername) === 0 ||
        (string)$er['created_by_user_id'] === (string)$currentUserId) {
      $canEdit = true;
    }
    ...
    if (!$canEdit) {
      ...
      if ($editCheckStmt) {
        $requestTypeTmp = (string)($er['request_type'] ?? '');
        $companyTmp = (string)($er['company'] ?? '');
        $uidStr = (string)$currentUserId;
        
        mysqli_stmt_bind_param($editCheckStmt, 'ssssssss', $requestTypeTmp, $companyTmp, $currentUserFullName, $currentUserUsername, $uidStr, $currentUserFirstName, $currentUserLastName, $currentUserFullName);
  ...
  // If user doesn't have edit permission, redirect to view page
  if (!$canEdit) {
    header('Location: disbursement_approver_view.php?id=' . $editId);
    exit;
  }
  
  // Log approver opening edit form
  if ($canEdit && $er && isset($er['request_type']) && isset($er['doc_number'])) {
    try {
      // Get workflow type ID
      $workflowTypeId = null;
      $typeStmt = mysqli_prepare($mysqlconn, "SELECT id FROM work_flow_type WHERE type_code = ? LIMIT 1");
      if ($typeStmt) {
        mysqli_stmt_bind_param($typeStmt, 's', $er['request_type']);
        mysqli_stmt_execute($typeStmt);
        $typeResult = mysqli_stmt_get_result($typeStmt);
        if ($typeRow = mysqli_fetch_assoc($typeResult)) {
          $workflowTypeId = (int)$typeRow['id'];
        }
      ...
      // Determine approver sequence
      $approverSequence = null;
      if ($currentUserId > 0) {
        $checkApproverStmt = mysqli_prepare($mysqlconn, "SELECT sequence FROM work_flow_template wft
                                                       WHERE wft.work_flow_id = ? 
                                                         AND wft.company = ? 
        ...
        if ($checkApproverStmt) {
          $uidStr = (string)$currentUserId;
          mysqli_stmt_bind_param($checkApproverStmt, 'ssssssss', $er['request_type'], $er['company'], $currentUserFullName, $currentUserUsername, $uidStr, $currentUserFirstName, $currentUserLastName, $currentUserFullName);
          mysqli_stmt_execute($checkApproverStmt);
          $approverRes = mysqli_stmt_get_result($checkApproverStmt);
          if ($approverRow = mysqli_fetch_assoc($approverRes)) {
            $approverSequence = (int)$approverRow['sequence'];
          }
          mysqli_stmt_close($checkApproverStmt);
        }
      }
      
      // Log edit form access
      if ($approverSequence) {
        error_log("Attempting to log edit form access for doc: " . $er['doc_number'] . ", sequence: $approverSequence, user: $currentUserFullName");
        $logStmt = mysqli_prepare($mysqlconn, "INSERT INTO work_flow_action_log 
          (doc_type, work_flow_type_id, doc_number, sequence, actor_id, action, event, prev_status, new_status, remarks, created_by, ip_address, created_at) 
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        
        if ($logStmt) {
          $event = 'EDIT_FORM_ACCESS';
          $action = 'Approver Edit Form Access';
          $remarks = "Approver opened edit form for document";
          $createdBy = $currentUserUsername;
          
          mysqli_stmt_bind_param($logStmt, 'sisisssssssss', 
            $er['request_type'], $workflowTypeId, $er['doc_number'], $approverSequence, $currentUserFullName, $action, $event, 
            null, null, $remarks, $createdBy, $ip_address);
          $result = mysqli_stmt_execute($logStmt);
          if (!$result) {
            error_log("Failed to execute work_flow_action_log insert in edit form: " . mysqli_error($mysqlconn));
          } else {
            error_log("Successfully logged edit form access for doc: " . $er['doc_number']);
          }
      ...
      error_log("Error logging edit form access: " . $e->getMessage());
    }
  }
          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Disbursement Approver Edit Form</h3>
              </div>
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Header Details <small>input the header details of your disbursement request.</small></h2>
                    ...
                    <form id="demo-form2" class="form-horizontal form-label-left" method="post" enctype="multipart/form-data" autocomplete="off" action="<?php echo ($editId > 0 ? 'update_financial_request.php' : 'save_financial_request.php'); ?>" novalidate onsubmit="restoreAllStoredFiles();">
                          ...
                          <template id="itemRowTemplate">
                            <div class="well item-row" style="padding:10px; position:relative;">
                              ...
                              <div class="form-group">
                                ...
                                <div class="col-md-2 col-sm-2 col-xs-12 field-ap-attachment field_ap_attachment_col" id="field_ap_attachment_col">
                                  <label>AP Attachment</label>
                                  <div class="upload-area" style="border: 2px dashed #ccc; border-radius: 8px; padding: 20px; text-align: center; background-color: #fafafa; transition: all 0.3s ease; cursor: pointer;" onmouseover="this.style.borderColor='#007bff'; this.style.backgroundColor='#f0f8ff';" onmouseout="this.style.borderColor='#ccc'; this.style.backgroundColor='#fafafa';">
                                    <i class="fa fa-cloud-upload" style="font-size: 24px; color: #666; margin-bottom: 10px;"></i>
                                    <div style="font-size: 12px; color: #666; margin-bottom: 5px;">Click to upload or drag files here</div>
                                    <div style="font-size: 10px; color: #999;">PDF, DOC, XLS, Images</div>
                                    <input type="file" name="items_ap_attachment_item_INDEX_" class="form-control item-ap-attachment-input" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.gif" multiple style="position: absolute; opacity: 0; width: 100%; height: 100%; cursor: pointer; top: 0; left: 0;" onchange="handleItemFileSelection(this)">
                                  </div>
                                  <div class="file-list" style="margin-top: 10px; font-size: 12px;"></div>
                                  <i class="form-control-feedback glyphicon glyphicon-asterisk" data-bv-icon-for="items_ap_attachment" style=""></i>
                                </div>
                                <div class="col-md-2 col-sm-2 col-xs-12 field-cv-attachment field_cv_attachment_col" id="field_cv_attachment_col">
                                  <label>CV Attachment</label>
                                  <div class="upload-area" style="border: 2px dashed #ccc; border-radius: 8px; padding: 20px; text-align: center; background-color: #fafafa; transition: all 0.3s ease; cursor: pointer;" onmouseover="this.style.borderColor='#007bff'; this.style.backgroundColor='#f0f8ff';" onmouseout="this.style.borderColor='#ccc'; this.style.backgroundColor='#fafafa';">
                                    <i class="fa fa-cloud-upload" style="font-size: 24px; color: #666; margin-bottom: 10px;"></i>
                                    <div style="font-size: 12px; color: #666; margin-bottom: 5px;">Click to upload or drag files here</div>
                                    <div style="font-size: 10px; color: #999;">PDF, DOC, XLS, Images</div>
                                    <input type="file" name="items_cv_attachment_item_INDEX_" class="form-control item-cv-attachment-input" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.gif" multiple style="position: absolute; opacity: 0; width: 100%; height: 100%; cursor: pointer; top: 0; left: 0;" onchange="handleItemFileSelection(this)">
                                  </div>
                                  <div class="file-list" style="margin-top: 10px; font-size: 12px;"></div>
                                  <i class="form-control-feedback glyphicon glyphicon-asterisk" data-bv-icon-for="items_cv_attachment" style=""></i>
                                </div>
                              ...
                              <div class="form-group">
                                ...
                                <div class="col-md-3 col-sm-3 col-xs-12 field-withholding-tax field_withholding_tax_col" id="field_withholding_tax_col">
                                  <label>Withholding Tax/Final Tax 
                                    <a href="javascript:void(0);" onclick="openAddWithholdingTaxModal()" 
                                       style="font-size: 11px; font-weight: normal; color: #5cb85c; text-decoration: none; margin-left: 8px;"
                                       onmouseover="this.style.color='#449d44'; this.style.textDecoration='underline';"
                                       onmouseout="this.style.color='#5cb85c'; this.style.textDecoration='none';"
                                       title="Add new withholding tax code">
                                      <i class="fa fa-plus-circle"></i> add new
                                    </a>
                                  </label>
                                  <select name="items_withholding_tax[]" class="form-control tax-calc-field">
                                    <option value="">Choose..</option>
                            ...
                            document.addEventListener('DOMContentLoaded', function(){
                              try {
                                ...
                                data.forEach(function(it){
                                  ...
                                  // Set the correct file input names with item index
                                  ...
                                  fileInputs.forEach(function(fileInput, index) {
                                    if (index === 0) {
                                      fileInput.name = 'items_attachment_item_' + itemIndex;
                                      fileInput.classList.add('item-attachment-input');
                                    } else if (index === 1) {
                                      fileInput.name = 'items_ap_attachment_item_' + itemIndex;
                                      fileInput.classList.add('item-ap-attachment-input');
                                    } else if (index === 2) {
                                      fileInput.name = 'items_cv_attachment_item_' + itemIndex;
                                      fileInput.classList.add('item-cv-attachment-input');
                                    }
                                  });
                                  
                                  // Set unique IDs for the upload areas
                                  var uploadAreas = root.querySelectorAll('.upload-area');
                                  uploadAreas.forEach(function(uploadArea, index) {
                                    if (!uploadArea.id) {
                                      uploadArea.id = 'item-upload-existing-' + itemIndex + '-' + index;
                                    }
                                  });
                      ...
                      <div class="form-group">
                        <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                          <a href="disbursement_approver_main.php" class="btn btn-primary">Back</a>
...
<script src="sweetalert2@11.js"></script>
    ...
    <!-- Include Add Withholding Tax Modal -->
    <?php include 'modal_add_withholding_tax.php'; ?>
    
    <script>
      // Function to open add withholding tax modal
      function openAddWithholdingTaxModal() {
        $('#addWithholdingTaxModal').modal('show');
      }
    </script>
      ...
      (function(){
        function enforceAllRequired(formEl){
          ...
          for (var i=0;i<fields.length;i++){
            ...
            if (el.closest && el.closest('#field_ap_attachment_col')) continue; // Do not require AP attachments
            if (el.closest && el.closest('#field_cv_attachment_col')) continue; // Do not require CV attachments
            if (el.closest && el.closest('#field_supporting_document_col')) continue; // Do not require supporting documents
        ...
        document.addEventListener('DOMContentLoaded', function(){
          ...
          function fixItemAttachmentNames() {
            ...
            allItemRows.forEach(function(row, index) {
              ...
              fileInputs.forEach(function(fileInput, inputIndex) {
                if (inputIndex === 0) {
                  fileInput.name = 'items_attachment_item_' + index;
                  fileInput.classList.add('item-attachment-input');
                } else if (inputIndex === 1) {
                  fileInput.name = 'items_ap_attachment_item_' + index;
                  fileInput.classList.add('item-ap-attachment-input');
                } else if (inputIndex === 2) {
                  fileInput.name = 'items_cv_attachment_item_' + index;
                  fileInput.classList.add('item-cv-attachment-input');
                }
              });
              
              // Set unique IDs for the upload areas
              var uploadAreas = row.querySelectorAll('.upload-area');
              uploadAreas.forEach(function(uploadArea, areaIndex) {
                if (!uploadArea.id) {
                  uploadArea.id = 'item-upload-' + index + '-' + areaIndex;
                }
              });
            });
          }
